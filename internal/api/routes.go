...
func (s *Server) registerRoutes(h *hertzserver.Hertz) {
	...
	api := h.Group("/api")
	{
		...
		api.POST("/workspace/upload-raw", apiHandlers.HandleWorkspaceUploadRaw)
		...
		api.GET("/books/deleted", apiHandlers.HandleDeletedBooks)
		...
		api.GET("/books/export", apiHandlers.HandleProjectExport)
		api.POST("/books/import-project", apiHandlers.HandleProjectImport)
		api.POST("/books/remove", apiHandlers.HandleBookRemove)
		api.POST("/books/restore", apiHandlers.HandleBookRestore)
		api.POST("/books/purge", apiHandlers.HandleBookPurge)
		...
		api.POST("/settings/test-model", apiHandlers.HandleSettingsTestModel)
		api.GET("/backup/export", apiHandlers.HandleDataBackupExport)
		api.POST("/backup/restore", apiHandlers.HandleDataBackupRestore)
